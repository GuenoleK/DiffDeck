...
export function PublicationQueue({ approvedFindings, onShare, review }: PublicationQueueProps) {
  ...
  const prompt = useMemo(() => {
    ...
    return [
      ...
      "If I want my real authenticated browser, prefer Playwright MCP + Chrome Extension. Opening the URL in my default browser is not enough; the agent must be able to inspect, click, hover, type, and place comments in that browser.",
      ...
      "After each saved comment or draft, verify that the thread/comment exists before moving to the next finding.",
      "Never publish or submit anything unless I explicitly confirm level 3.",
      "Treat Chrome DevTools MCP as diagnostic or limited fallback for GitLab inline comments. On large virtualized diffs, avoid JavaScript injection, DOM mutation scripts, and generic evaluate_script; prefer snapshots, locator/accessibility clicks, hover, scroll, keyboard input, and text filling.",
      ...
      "If you cannot disconnect the browser connection from your side, tell me exactly how to do it for the current mode, for example stopping Playwright MCP, disconnecting the extension session, closing the Playwright browser, disabling remote debugging, closing a dedicated debugging browser, stopping DevTools MCP, or closing the integrated browser session.",
    ].join("\n");
